...
const MyAppointments = () => {
  ...
  },[token])  // in dependency array add token 
    ...
    <div>
      ...
      {!!appointments.length && appointments.map((item,index)=>(
        <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
          <div>
            <img className='w-32 bg-indigo-50' src={item.herbData.image} alt="" />
          </div>
          <div className='flex-1 text-sm text-zinc-600'>
            <p className='text-neutral-800 font-semibold'>{item.herbData.name}</p>
            <p>{item.herbData.speciality}</p>
            <p className='text-zinc-700 font-medium mt-1'>Address:</p>
            {/* <p className='text-xs'>{item.herbData.address.line1}</p>
            <p className='text-xs'>{item.herbData.address.line2}</p> */}
